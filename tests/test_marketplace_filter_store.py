...
def _grant(conn, *, group_id, marketplace, plugin):
    ...
    ResourceGrantsRepository(conn).create(
        group_id=group_id, resource_type="marketplace_plugin",
...
class TestResolveUserMarketplace:
    ...
    def test_multiple_skills_share_one_bundle(self, db_conn):
        ...
        e1 = _create_store_entity(db_conn, owner_id="owner", owner_username="owner",
                                  name="alpha", type_="skill")
        e2 = _create_store_entity(db_conn, owner_id="owner", owner_username="owner",
                                  name="beta", type_="skill")
    ...
    def test_skill_and_agent_share_bundle(self, db_conn):
        ...
        _make_user(db_conn, user_id="owner", email="owner@x")
        _make_user(db_conn, user_id="u1", email="u1@x")
        s = _create_store_entity(db_conn, owner_id="owner", owner_username="owner",
                                 name="my-skill", type_="skill")
        a = _create_store_entity(db_conn, owner_id="owner", owner_username="owner",
                                 name="my-agent", type_="agent")
    ...
    def test_plugin_entity_stays_standalone(self, db_conn):
        ...
        _make_user(db_conn, user_id="owner", email="owner@x")
        _make_user(db_conn, user_id="u1", email="u1@x")
        plugin_eid = _create_store_entity(db_conn, owner_id="owner",
                                          owner_username="owner",
                                          name="my-plugin", type_="plugin")
    ...
    def test_mixed_plugin_and_skill_two_entries(self, db_conn):
        ...
        _make_user(db_conn, user_id="owner", email="owner@x")
        _make_user(db_conn, user_id="u1", email="u1@x")
        p_eid = _create_store_entity(db_conn, owner_id="owner",
                                     owner_username="owner",
                                     name="my-plugin", type_="plugin")
        s_eid = _create_store_entity(db_conn, owner_id="owner",
                                     owner_username="owner",
                                     name="my-skill", type_="skill")
