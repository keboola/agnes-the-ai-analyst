...
# ─────────────────────── atomic publish (#1359) ────────────────────────────
#
# merge_parquet and connectors/keboola/partitioned.py::merge_partition were a
...
# pass even against code that isn't atomic at all, since a no-op boom never
# gets far enough to prove anything about the write.
...
def test_merge_killed_write_leaves_no_temp_behind(tmp_path, monkeypatch):
    from connectors.keboola.incremental import merge_parquet
...
def test_merge_published_mode_is_0644_under_restrictive_umask(tmp_path):
    import os
...
def test_merge_temp_is_per_process_and_never_matches_the_parquet_glob(tmp_path, monkeypatch):
    import os

    from connectors.keboola import incremental as incremental_mod
    ...
    seen: list[Path] = []
    real = incremental_mod.pq.write_table

    def _record(table, where, **kw):
        seen.append(Path(where))
        return real(table, where, **kw)

    monkeypatch.setattr(incremental_mod.pq, "write_table", _record)
    incremental_mod.merge_parquet(
        existing_parquet=pq_path,
        new_csv=csv_path,
        primary_key=["id"],
        dtypes={"id": "Int64"},
    ...
    assert seen[0].name != "t.parquet", "wrote straight onto the live path"
    assert str(os.getpid()) in seen[0].name
    assert [p.name for p in tmp_path.glob("*.parquet")] == ["t.parquet"]
