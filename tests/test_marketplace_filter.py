...
class TestResolveUserMarketplace:
    """``resolve_user_marketplace`` composes the actual served set per user:
    ``(rbac ∩ subscriptions) ∪ store_installs``. The aggregate-side admin
    god-mode parity lives here, NOT in ``resolve_allowed_plugins`` (which
    stays strict — see ``TestResolveAllowedPlugins`` above).
    """

    def test_admin_subscriptions_surface_without_explicit_grants(self, db_conn):
        """Regression: admin user POSTs ``/curated/<m>/<p>/install`` and the
        install endpoint's ``require_resource_access`` → ``can_access``
        gate accepts it via admin god-mode, even when no explicit
        ``resource_grant`` exists for Admin. The subscription row
        persists, and the served ``/marketplace.git/`` clone must
        surface the plugin — otherwise admins see ``installed: true``
        in ``agnes marketplace search`` but the actual aggregate clone
        is empty.

        Match the install-side semantic: admins bypass the eligibility
        filter inside ``resolve_user_marketplace``. Non-admins still
        need explicit grants to enter the served set.
        """
        ...
        t = datetime.now(timezone.utc)
        _register_marketplace(db_conn, id="mkt-x", registered_at=t,
            plugins=[{"name": "px", "version": "1.0"}])
        _make_user(db_conn, user_id="u-admin2", email="admin2@x")
        admin_gid = db_conn.execute(
            "SELECT id FROM user_groups WHERE name='Admin'"
        ).fetchone()[0]
        _add_member(db_conn, user_id="u-admin2", group_id=admin_gid)
        # No explicit grant on Admin group — admin god-mode is the only
        # path. Insert the subscription as the install endpoint would.
        UserCuratedSubscriptionsRepository(db_conn).subscribe(
            "u-admin2", "mkt-x", "px",
        )

        result = resolve_user_marketplace(db_conn, {"id": "u-admin2"})
        prefixed = {p["prefixed_name"] for p in result}
        assert "mkt-x-px" in prefixed, (
            "admin subscription must reach served set despite no explicit grant"
        )

    def test_non_admin_still_needs_explicit_grant(self, db_conn):
        """Counterpart: a non-admin user with a subscription but no
        explicit ``resource_grant`` sees nothing in the served set.
        The eligibility filter still applies.
        """
        from src.marketplace_filter import resolve_user_marketplace
        from src.repositories.user_curated_subscriptions import (
            UserCuratedSubscriptionsRepository,
        ...
        t = datetime.now(timezone.utc)
        _register_marketplace(db_conn, id="mkt-y", registered_at=t,
            plugins=[{"name": "py", "version": "1.0"}])
        _make_user(db_conn, user_id="u-regular", email="reg@x")
        gid = _make_group(db_conn, name="G-regular")
        _add_member(db_conn, user_id="u-regular", group_id=gid)
        # Subscription exists but no resource_grant — non-admin should
        # NOT see the plugin in their served set.
        UserCuratedSubscriptionsRepository(db_conn).subscribe(
            "u-regular", "mkt-y", "py",
        )

        result = resolve_user_marketplace(db_conn, {"id": "u-regular"})
        prefixed = {p["prefixed_name"] for p in result}
        assert "mkt-y-py" not in prefixed
