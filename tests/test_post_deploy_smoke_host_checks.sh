...
run_script() {
    ...
    out=$(AGNES_OPT_DIR="$OPT_DIR" AGNES_URL=http://smoke.test \
          ...
          DOCTOR_EMAIL_TO="${DOCTOR_EMAIL_TO:-}" \
          bash "$script" 2>&1)
    rc=$?
    set -e
}
...
new_scenario() {
    ...
    : > "$transcript"
    unset FAKE_WHEEL_HTTP FAKE_DOCTOR_JSON DOCTOR_EMAIL_TO || true
}
...
# A quote in the recipient must reach the doctor as escaped JSON, not malform
# the payload (the body is built with json.dumps, not printf interpolation).
DOCTOR_EMAIL_TO='ops"weird@example.org'
run_script
...
grep -F 'ops\"weird@example.org' "$transcript" >/dev/null || fail "E: email_to not JSON-escaped in the request body"
echo "scenario E ok"
