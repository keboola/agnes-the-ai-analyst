...
def _seed_item(
    *,
    ...
    status: str = "pending",
    category: str = "fact",
    source_user: str = "admin@test.com",
...
def test_domains_reflects_seeded_domain(seeded_app_both):
    ...
    r = seeded_app_both["client"].get("/api/memory/domains", headers=_auth(seeded_app_both))
    assert r.status_code == 200, r.text
    slugs = {d["slug"] for d in r.json()["domains"]}
    assert "md_probe" in slugs, f"[{seeded_app_both['backend']}] seeded domain missing from /domains: {r.json()}"
...
def test_admin_pending_reflects_seeded_pending_item(seeded_app_both):
    ...
    r = seeded_app_both["client"].get("/api/memory/admin/pending", headers=_auth(seeded_app_both))
    assert r.status_code == 200, r.text
    titles = {it["title"] for it in r.json()["items"]}
    assert "Pending Probe" in titles, (
        f"[{seeded_app_both['backend']}] seeded pending item missing from /admin/pending: {r.json()}"
    )
...
def test_my_contributions_reflects_seeded_item(seeded_app_both):
    ...
    r = seeded_app_both["client"].get("/api/memory/my-contributions", headers=_auth(seeded_app_both))
    assert r.status_code == 200, r.text
    titles = {it["title"] for it in r.json()["items"]}
    assert "My Contribution Probe" in titles, (
        f"[{seeded_app_both['backend']}] seeded contribution missing from /my-contributions: {r.json()}"
    )
...
def test_stats_total_reflects_seeded_item(seeded_app_both):
    ...
    r = seeded_app_both["client"].get("/api/memory/stats", headers=_auth(seeded_app_both))
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["total"] >= 1, (
    ...
    assert "probecat" in body["categories"], (
        f"[{seeded_app_both['backend']}] seeded category missing from /stats categories: {body['categories']}"
    )
...
def test_tree_reflects_seeded_item(seeded_app_both):
    ...
    r = seeded_app_both["client"].get("/api/memory/tree?axis=category", headers=_auth(seeded_app_both))
    assert r.status_code == 200, r.text
    body = r.json()
    ...
    all_titles = {it["title"] for g in body["groups"] for it in g["items"]}
    assert "Tree Probe" in all_titles, f"[{seeded_app_both['backend']}] seeded item missing from /tree groups: {body}"


# ---------------------------------------------------------------------------
# _resolve_domain_slug — parity contract (DuckDB + PG must both implement it)
# ---------------------------------------------------------------------------


def test_resolve_domain_slug_known(seeded_app_both):
    """Both backends return the domain id for a known slug."""
    _seed_domain("slug_probe", "Slug Probe Domain")
    from src.repositories import knowledge_repo

    result = knowledge_repo()._resolve_domain_slug("slug_probe")
    assert result is not None, f"[{seeded_app_both['backend']}] _resolve_domain_slug returned None for known slug"


def test_resolve_domain_slug_unknown(seeded_app_both):
    """Both backends return None for an unknown slug."""
    from src.repositories import knowledge_repo

    result = knowledge_repo()._resolve_domain_slug("nonexistent_slug_xyz")
    assert result is None, f"[{seeded_app_both['backend']}] _resolve_domain_slug returned {result!r} for unknown slug"
