...
def test_pg_day_bucketing_is_utc_regardless_of_session_timezone(pg_repo_skewed_tz):
    ...
    # reports_pg window boundaries: "today" must be the UTC date, not the
    # session-TZ CURRENT_DATE. Seed the daily fact at the two positions that
    # discriminate a ±1-day boundary shift: utc_today-7 belongs to the recent
    # window, utc_today-8 to the prior window. A boundary shifted either way
    # misclassifies one of them, so trend_pct != 0.
    from src.repositories.reports_pg import ReportsPgRepository

    reports = ReportsPgRepository(repo._engine)
    with repo._engine.begin() as c:
        for day in (utc_day - timedelta(days=7), utc_day - timedelta(days=8)):
            c.execute(
                sa.text(
                    "INSERT INTO usage_marketplace_item_daily "
                    "(day, source, type, parent_plugin, name, count, distinct_users, error_count) "
                    "VALUES (:day, 'curated', 'plugin', '', 'trendplug', 4, 1, 0)"
                ),
                {"day": day},
            )
    stats = reports.invocation_stats("curated")
    assert stats["trendplug"]["trend_pct"] == pytest.approx(0.0)
