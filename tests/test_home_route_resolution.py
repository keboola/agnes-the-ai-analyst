...
def test_home_renders_manual_gws_branch_when_unset(fresh_db, monkeypatch):
    ...
    assert resp.status_code == 200
    body = resp.text
    # Unconfigured branch renders: operator path offered first, manual
    # walkthrough gated behind an explicit choice (seed 0b6bebf)
    assert "FIRST, offer the operator path" in body
