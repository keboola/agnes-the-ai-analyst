...
def test_every_step_anchor_exists_in_templates() -> None:
    ...
    missing = [s.key for s in ONBOARDING_STEPS if s.anchor and f'data-tour="{s.anchor}"' not in blob]
    assert not missing, (
...
def test_steps_are_server_injected_not_hardcoded_in_js() -> None:
    ...
    The legacy (default-chrome) tour lives in the frozen `_tour_legacy.html`
    + `js/tour_legacy.js` pair (spec 2026-08-07 wave 2); the rail chrome's
    coach-mark engine has its own suite (test_tour_onboarding_steps.py)."""
    partial = (TEMPLATES / "_tour_legacy.html").read_text(encoding="utf-8")
    assert "onboarding_steps(" in partial, "_tour_legacy.html must inject server-filtered steps"
    assert 'id="agnesOnboardingSteps"' in partial

    engine = (STATIC / "js" / "tour_legacy.js").read_text(encoding="utf-8")
    assert "agnesOnboardingSteps" in engine, "tour_legacy.js must read the injected steps"
...
def test_engine_resumes_across_page_navigation() -> None:
    ...
    engine = (STATIC / "js" / "tour_legacy.js").read_text(encoding="utf-8")
    assert "sessionStorage" in engine, "tour_legacy.js must persist its resume position"
    assert "window.location.assign" in engine, "tour_legacy.js must navigate between step pages"


# Frozen byte-for-byte copies of the pre-redesign tour assets — the static
# sibling of tests/test_no_ui_emoji.py's LEGACY_FROZEN template set. A CLOSED
# set on purpose: a new script cannot take a `_legacy` name to dodge review
# of what it freezes; extend deliberately or not at all.
LEGACY_FROZEN_STATIC: set[str] = {"js/tour_legacy.js", "css/tour_legacy.css"}
...
def test_legacy_static_assets_are_a_closed_set() -> None:
    ...
    assert on_disk <= LEGACY_FROZEN_STATIC, (
        "unexpected *_legacy static assets (extend LEGACY_FROZEN_STATIC "
        f"deliberately if this is a new frozen copy): {sorted(on_disk - LEGACY_FROZEN_STATIC)}"
    )
    assert LEGACY_FROZEN_STATIC <= on_disk, (
        f"frozen static asset missing from disk: {sorted(LEGACY_FROZEN_STATIC - on_disk)}"
    )
