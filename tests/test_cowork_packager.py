...
class TestSkillFrontmatter:
    ...
    def test_bracket_folder_name_stays_string(self):
        # A folder like `[id]` must emit as a quoted string, not a YAML list.
        out = cp.filter_skill_frontmatter("---\ndescription: d\n---\nb\n", "dyn-id")
        assert yaml.safe_load(out.split("---")[1])["name"] == "dyn-id"
...
class TestBuildCoworkZip:
    ...
    def test_symlink_to_host_file_excluded(self, tmp_path):
        ...
        plugin = _make_plugin(tmp_path)
        # Hostile symlink inside the plugin pointing at a host file.
        os.symlink(secret, plugin["plugin_dir"] / "leak.env")
        files = _read_zip(cp.build_cowork_zip(plugin)[0])
        assert "leak.env" not in files
        assert not any(b"supersecret" in v for v in files.values())

    def test_arcname_collision_deduped(self, tmp_path):
        # `[id]` and `dyn-id` skill dirs both sanitize to `skills/dyn-id/` —
        # the zip must keep both SKILL.md under distinct arcnames.
        plugin = _make_plugin(tmp_path)
        d = plugin["plugin_dir"]
        (d / "skills" / "[id]").mkdir(parents=True)
        (d / "skills" / "[id]" / "SKILL.md").write_text(
            "---\nname: a\ndescription: bracket one\n---\nb\n", encoding="utf-8"
        )
        (d / "skills" / "dyn-id").mkdir(parents=True)
        (d / "skills" / "dyn-id" / "SKILL.md").write_text(
