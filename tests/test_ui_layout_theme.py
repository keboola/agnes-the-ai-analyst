...
class TestPaperThemeAssets:
    ...
    def test_trustmark_css_is_deliberately_global_not_scoped(self):
        """The one redesign sheet that is INTENTIONALLY unscoped, pinned as such.

        Every other sheet here is opt-in and must be inert on a default
        blue/topnav instance. `trustmark.css` is the exception on purpose: the
        trust markers are a default-look change, shipped knowingly (see the
        CHANGELOG bullet "Every Library row states its provenance, and it does
        so by default", which calls out that instances which never opted in
        will now see markers on rows that had none). The vocabulary has to be
        legible on every theme, so scoping it to paper would leave blue
        instances rendering unstyled markers.

        This test exists because NOTHING asserted the sheet either way, so an
        unscoped rule here was indistinguishable from an accident — and the
        sibling tests above would tempt a future reader into "fixing" it. If
        the markers ever become opt-in, invert this test rather than deleting
        it.
        """
        selectors = self._selectors("app/web/static/css/trustmark.css")
        assert selectors, "selector scan returned nothing — the regex broke"
        assert not any('[data-theme="paper"]' in sel for sel in selectors), (
            "trustmark.css has become theme-scoped. That is a real product "
            "change (the markers would stop rendering on blue/navy/dark), not "
            "a cleanup — update the CHANGELOG and invert this test if intended."
        )
