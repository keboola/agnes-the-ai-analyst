...
def test_when_current_short_circuits_no_install():
    with (
        patch("cli.commands.self_upgrade.check", return_value=_current_info()),
...
def test_uv_path_when_uv_available():
    # With the staged-first fix the flow is: curl download → uv tool install
    # <local-path> → (on success) record cache. The first subprocess call is
    # the staging curl; the install call uses the local staged wheel path.
    with (
        patch("cli.commands.self_upgrade.check", return_value=_outdated_info()),
        ...
        # A uv tool install call must appear (install from staged local wheel).
        uv_installs = [cmd for cmd in all_cmds if len(cmd) >= 3 and cmd[:3] == ["uv", "tool", "install"]]
        assert uv_installs, f"expected a 'uv tool install' call, got: {all_cmds}"
        uv_cmd = uv_installs[0]
        assert "--force" in uv_cmd
        # The staged path ends in a .whl file (local install, not the raw URL).
        whl_args = [a for a in uv_cmd if a.endswith(".whl")]
        assert whl_args, f"expected a .whl path argument in uv install call: {uv_cmd}"
...
def test_uv_available_but_python_outside_uv_tools_uses_pip(monkeypatch):
    ...
    with (
        patch("cli.commands.self_upgrade.check", return_value=_outdated_info()),
        patch(
            "cli.commands.self_upgrade.shutil.which",
        ...
        assert result.exit_code == 0
        cmds = [c.args[0] for c in mock_run.call_args_list]
        assert not any(len(cmd) >= 3 and cmd[:3] == ["uv", "tool", "install"] for cmd in cmds), (
...
def test_force_invalidates_cache_before_check():
    ...
    fresh_current_with_url = UpdateInfo(installed="0.40.0", latest="0.40.0", download_url=_OUTDATED_URL)
    with (
        patch("cli.commands.self_upgrade._invalidate_update_cache") as mock_invalidate,
...
def test_smoke_fail_triggers_rollback_when_prior_url_known():
    ...
    with (
        ...
        assert result.exit_code == 1
        urls_installed = [
            arg for c in mock_run.call_args_list for arg in c.args[0] if isinstance(arg, str) and arg.startswith("http")
        ]
...
def test_smoke_fail_with_rollback_failure_surfaces_rc():
    ...
    Call order with staged-first fix: curl (staging) → uv install (local) → uv rollback.
    """
    install_results = [
        MagicMock(returncode=0),  # staging curl download
        MagicMock(returncode=0),  # uv tool install (forward, from staged local wheel)
        MagicMock(returncode=99),  # uv tool install (rollback via URL)
    ]
    with (
        patch("cli.commands.self_upgrade.check", return_value=_outdated_info()),
        patch("cli.commands.self_upgrade.shutil.which", return_value="/usr/local/bin/uv"),
...
def test_smoke_pass_records_last_known_good_then_invalidates_cache():
    ...
    with (
        ...
        patch(
            "cli.commands.self_upgrade._record_last_known_good_meta",
        ...
        patch(
            "cli.commands.self_upgrade._invalidate_update_cache",
        ...
        invalidate_idx = max(i for i, c in enumerate(call_order) if c[0] == "invalidate")
...
def test_self_upgrade_propagates_sentinel_to_smoke_subprocess():
    ...
    def _fake_smoke(method, expected_version, *, user=False):
        env = {**os.environ, "AGNES_NO_UPDATE_CHECK": "1", "AGNES_SELF_UPGRADE_IN_PROGRESS": "1"}
...
def test_smoke_test_detects_version_mismatch(install_method, patch_target):
    ...
    with patch.object(su, patch_target, return_value=Path(fake_bin)), patch.object(su.subprocess, "run") as mock_run:
...
def test_self_upgrade_bypasses_24h_cache_without_force(tmp_path, monkeypatch):
    ...
    assert info.latest == "0.47.2", (
        f"expected fresh probe to return 0.47.2; got {info.latest} (cache short-circuit regressed)"
    )
    assert info.installed == "0.47.1"
    assert info.download_url == ("http://server.test/cli/wheel/agnes-0.47.2-py3-none-any.whl")
...
def test_smoke_test_passes_with_pep440_local_version():
    ...
    with (
        patch.object(su, "_uv_tool_bin_path", return_value=Path("/fake/agnes")),
...
def test_hook_refresh_failure_does_not_flip_exit_code(monkeypatch):
    ...
    with (
        patch("cli.commands.self_upgrade.check", return_value=_current_info()),
        patch(
            "cli.commands.self_upgrade.maybe_refresh_claude_hooks",
...
def test_python_is_uv_tool_install_sys_prefix_under_uv_root(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "cli.commands.self_upgrade.shutil.which", lambda name: "/usr/local/bin/uv" if name == "uv" else None
    )
    monkeypatch.setattr(
...
def test_user_method_passes_user_flag(monkeypatch):
    ...
    with (
        patch("cli.commands.self_upgrade.check", return_value=_outdated_info()),
        patch("cli.commands.self_upgrade._classify_install_method", return_value=("user", {"user": True})),
        patch("cli.commands.self_upgrade.subprocess.run") as mock_run,
        patch("cli.commands.self_upgrade._smoke_test_new_binary", return_value=_smoke_pass()),
        patch("cli.commands.self_upgrade._read_last_known_good", return_value=None),
        patch("cli.commands.self_upgrade._record_last_known_good"),
...
def test_preflight_defers_unattended_when_artifact_missing(tmp_path):
    ...
    prior_meta = {
        "download_url": _PRIOR_URL,
        "version": "0.35.0",
        "wheel_filename": "0.35.0.whl",
        "sha256": "deadbeef",
    }  # file absent
    with (
        patch("cli.commands.self_upgrade.check", return_value=_outdated_info()),
...
def test_windows_deferred_staging_failure_is_failsafe(monkeypatch):
    ...
    monkeypatch.setattr("cli.commands.self_upgrade.sys.platform", "win32")
    monkeypatch.setattr(
        "cli.commands.self_upgrade._spawn_windows_deferred_update", lambda info, prior_meta, *, quiet: False
    )
    with (
        patch("cli.commands.self_upgrade.check", return_value=_outdated_info()),
        patch("cli.commands.self_upgrade.shutil.which", return_value="uv.exe"),
...
def test_spawn_windows_deferred_update_builds_detached_popen(monkeypatch, tmp_path):
    ...
    info = UpdateInfo(
        installed="0.72.1", latest="0.72.2", download_url="http://s/agnes_the_ai_analyst-0.72.2-py3-none-any.whl"
    )
...
def test_spawn_windows_deferred_update_no_interpreter_fails_safe(monkeypatch, tmp_path):
    ...
    called = {"popen": 0}
    monkeypatch.setattr("cli.commands.self_upgrade.subprocess.Popen", lambda *a, **k: called.__setitem__("popen", 1))
