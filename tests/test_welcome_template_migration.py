...
v26 consolidated welcome_template + claude_md_template into the generic
`instance_templates(key, content, ...)` table; the legacy `welcome_template`
table is dropped during the v25 → v26 migration. The historical assertion
"a v20 DB upgrades and welcome_template exists" is no longer valid because the
ladder runs through v26 and consolidates the table away.

This file is preserved so the welcome-template migration history stays
covered in the test suite, but the assertion is reshaped to match v26
...
def test_welcome_content_landing_post_v26(tmp_path):
    """A fresh-install ladder lands the welcome row in instance_templates,
    not in a separate welcome_template table (consolidated in v26)."""
    ...
    # Legacy welcome_template table is gone post-v26.
    tables = {
        r[0]
    ...
    assert "welcome_template" not in tables, (
        "welcome_template should be consolidated into instance_templates by v26"
    )
