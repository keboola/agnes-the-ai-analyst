...
Covers:
  ...
  * GET response includes the virtual ``badge`` — `new` or None, derived
    server-side from ``created_at`` age. v113 retired `curated`: it was
    derived from the creator's CURRENT Admin-group membership, and the trust
    claim is now the stored ``publisher_kind`` the response also carries.
"""
