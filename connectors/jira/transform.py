...
# documented read-only projection of it. Not competing duplicates — prefer the
# flag, because it rides along on the comments embedded in a plain
# `GET /issue/{key}` (no `expand`, no second request, so this transform stays
# pure and the fetch layer is untouched), while the property needs
# `expand=properties`, is absent on ~24% of comments, and is not consistently
# typed. Audited 2022-2026 on a live instance: the flag on 100% of comments, the
# property on 76%, agreeing wherever both appeared. Reports of `jsdPublic` being
# wrong (JSDCLOUD-7997, -8275, -6050) concern WEBHOOK payloads and WRITE
# attempts; this connector only reads GET responses. Column semantics for
# consumers are in connectors/jira/README.md.
_PUBLIC_COMMENT_PROPERTY = "sd.public.comment"


def _comment_public_visibility(comment: dict) -> bool | None:
    """Is this comment customer-facing? ``None`` when neither signal is present.
    ...
    from an observed one. A sibling ingest of this same field defaulted instead:
    it holds zero NULLs across 120k rows, and spot-checking its rows against
    live Jira finds 25-30% of every pre-2025 month wrong, always in the same
    direction (stored public, actually internal).
    """
    ...
    for prop in comment.get("properties") or []:
        if not isinstance(prop, dict) or prop.get("key") != _PUBLIC_COMMENT_PROPERTY:
            continue
        value = prop.get("value")
        if not isinstance(value, dict) or "internal" not in value:
            continue
        raw = value["internal"]
        # NOT `bool(raw)`: this field is not consistently typed. The same Jira
        # instance stores both a JSON boolean and the STRING "false", and every
        # non-empty string is truthy — so the naive spelling silently flips a
        # public comment to internal.
        internal = raw.strip().lower() == "true" if isinstance(raw, str) else bool(raw)
        return not internal
...
def transform_comments(raw_issue: dict, *, preserve_on_incomplete: bool = True) -> list[dict] | None:
    ...
    if unresolved:
        ...
        logger.warning(
            "Jira issue %s: %d of %d comments carry neither jsdPublic nor an "
            "%s property — public_visibility written as NULL",
            issue_key,
            unresolved,
            len(comments),
            _PUBLIC_COMMENT_PROPERTY,
        )
