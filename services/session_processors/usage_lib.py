    ...
    def __init__(self):
        ...
        with get_engine().connect() as conn:
            curated_rows = conn.execute(
                sa.text("SELECT DISTINCT name FROM marketplace_plugins")
            ).fetchall()
            flea_rows = conn.execute(
                sa.text(
                    "SELECT name, type FROM store_entities "
                    "WHERE visibility_status = 'approved'"
                )
            ).fetchall()
        self._curated_plugins: set[str] = {r[0] for r in curated_rows}
        ...
        self._flea_entities: dict[str, str] = {r[0]: r[1] for r in flea_rows}
...
def _last_30d_due(conn) -> bool:
    ...
    ``conn`` is an SA ``Connection`` (engine.connect() / engine.begin()).
    """
    import sqlalchemy as sa
    row = conn.execute(
        sa.text(
...
def rebuild_rollups(*, since_day=None, force_30d: bool = False) -> None:
    ...
    All writes run inside a single ``engine.begin()`` transaction so a
    partial failure never leaves the rollup set inconsistent.
    """
    ...
    engine = get_engine()
    # Lookup-table reads happen outside the write transaction so the
    # event-fetch queries don't sit inside the same begin()-scoped row
    # locks taken by the DELETE statements below.
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            sa.text("DELETE FROM usage_tool_daily WHERE day >= :since"),
            {"since": since_day},
        )
        conn.execute(
            sa.text(
        ...
        # ---- usage_marketplace_item_daily (incremental last 7d) ----
        ...
        conn.execute(
            sa.text("DELETE FROM usage_marketplace_item_daily WHERE day >= :since"),
            {"since": since_day},
        )
        if daily_buckets:
            insert_daily = sa.text(
                """
                INSERT INTO usage_marketplace_item_daily
                    (day, source, type, parent_plugin, name, count, distinct_users, error_count)
                VALUES (:day, :source, :type, :parent, :name, :count, :distinct_users, :errors)
                """
            )
            conn.execute(
                insert_daily,
                [
                    {
                        "day": day, "source": source, "type": type_,
                        "parent": parent, "name": name,
                        "count": v["count"], "distinct_users": len(v["users"]),
                        "errors": v["errors"],
                    }
        ...
        # ---- usage_marketplace_item_window period_label='last_7d' ----
...
def _rebuild_window(conn, period_label: str, cutoff_day, curated_plugins, flea_entities) -> None:
    ...
    ``conn`` is an SA ``Connection`` from the caller's ``engine.begin()``
    block — this function only emits DML and never opens its own
    transaction.
    """
    import sqlalchemy as sa
    events = conn.execute(
    ...
    conn.execute(
        sa.text(
            "DELETE FROM usage_marketplace_item_window WHERE period_label = :p"
        ),
        {"p": period_label},
    )
    if buckets:
        insert_window = sa.text(
            """
            INSERT INTO usage_marketplace_item_window
                (period_label, source, type, parent_plugin, name, invocations, distinct_users)
            VALUES (:period, :source, :type, :parent, :name, :invocations, :distinct_users)
            """
        )
        conn.execute(
            insert_window,
            [
                {
                    "period": period_label, "source": source, "type": type_,
                    "parent": parent, "name": name,
                    "invocations": v["count"], "distinct_users": len(v["users"]),
                }
