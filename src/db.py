...
# v28: /home page rollout. Two changes bundled because they ship together.
# Originally drafted as v26 in the home-page branch but rebased over
# upstream's v26 (Keboola local→materialized) and v27 (sync-strategy
# columns), so the same logic now lands at v28.
#
...
#      legacy tables. Per Decision §B in the brainstorm — consolidate now
#      before a third copy ships.
#
#      The CREATE+seed pure-SQL portion lives in this list; the conditional
#      INSERT-from-legacy + DROP lives in _v27_to_v28_finalize() below because
#      it needs information_schema lookups to handle both fresh-install
#      (no legacy tables) and existing-v27 (legacy tables present) paths
#      cleanly.
#
...
#      self-mark button on the not-onboarded view) flips it to TRUE. Per
#      Decision §D: no PAT-heuristic auto-flip, no operator pre-flip recipe
#      shipped with the OSS — operators run their own UPDATE if they want.
...
def _v27_to_v28_finalize(conn) -> None:
    """Migrate legacy welcome_template + claude_md_template rows into
    instance_templates, then drop the legacy tables.

    Runs after _V27_TO_V28_MIGRATIONS creates the new table. Idempotent:
    re-running on an already-v28 DB is a no-op because the legacy tables
