...
# `_v118_to_v119`),
# 120 adds tool_grants.allow_mutating — per-group opt-in that lets a
...
def _v119_to_v120(conn: duckdb.DuckDBPyConnection) -> None:
    """v119→v120: add ``tool_grants.allow_mutating``.
    ...
    *before* any DDL runs — operators reading the snapshot for rollback
    ...
    if current < SCHEMA_VERSION:
        ...
        if current == 0:
            ...
            # installs — _SYSTEM_SCHEMA already declares the column.
            _v118_to_v119(conn)
            # v119→v120: tool_grants.allow_mutating. tool_grants is created
