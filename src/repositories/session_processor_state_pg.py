...
class SessionProcessorStatePgRepository:
    ...
    def reset_processor(self, processor_name: str) -> int:
        """Drop every state row for one processor. Returns rows deleted.

        Used by ``services.verification_detector --reset`` so the next
        invocation re-processes every session (e.g. after the LLM rules
        change and the operator wants a clean re-extraction).
        """
