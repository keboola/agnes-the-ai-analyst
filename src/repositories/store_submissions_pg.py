...
VALID_STATUSES = {
    ...
    "review_error",
    "overridden",
    # 'deleted' marks rows whose linked entity was hard-deleted (admin
    # DELETE ?hard=true). Explicit marker so the admin Deleted chip can
    # surface the row even though the JOIN-based filter can't reach it.
    "deleted",
    # 'archived' is DEPRECATED in writes. Post-v35 archive lifecycle reads
    # live from store_entities.visibility_status; kept in the validator
    # so historical rows from instances that ran the prior denormalised
    # path still pass.
    "archived",
}
...
class StoreSubmissionsPgRepository:
    ...
    def delete(self, id: str) -> None:
        """Hard-delete a submission row by id. Caller is responsible for
        removing any bundle / entity tied to it.
        """
        with self._engine.begin() as conn:
            conn.execute(
    ...
    def update_inline_review(
        self,
        id: str,
        *,
        status: str,
        inline_checks: Dict[str, Any],
    ) -> None:
        """Persist a fresh inline-review verdict for a submission.

        Resets ``llm_findings`` (the upstream verdict is no longer
        meaningful once inline has been re-evaluated) and bumps
        ``updated_at``. Used by the admin rescan flow when a previously
        blocked submission is re-checked against new inline rules.
        """
        if status not in VALID_STATUSES:
            raise ValueError(f"invalid submission status: {status!r}")
        with self._engine.begin() as conn:
            conn.execute(
                sa.text(
                    ...
                    "inline_checks = CAST(:ic AS JSONB), "
                    "llm_findings = NULL, "
                    "status = :status, "
                    "updated_at = :now "
                    "WHERE id = :id"
    ...
    def list_for_admin(
        ...
        *,
        ...
        if name_substr:
            ...
            clauses.append("LOWER(s.name) LIKE :name_substr ESCAPE '\\'")
            params["name_substr"] = f"%{pg_like_escape(name_substr.lower())}%"
        if version_substr:
            from src.sql_safe import pg_like_escape
            clauses.append(
                "LOWER(COALESCE(s.version, '')) LIKE :version_substr ESCAPE '\\'"
            )
            params["version_substr"] = f"%{pg_like_escape(version_substr.lower())}%"
