    ...
    def query(
        ...
        *,
        ...
        if action_prefix is not None:
            ...
            where.append("action LIKE :action_prefix ESCAPE '\\'")
            params["action_prefix"] = pg_like_escape(action_prefix) + "%"
        if action_in:
            in_keys: List[str] = []
        ...
        if result_pattern is not None:
            # ``result_pattern`` is operator-supplied (admin filter) and
            # intentionally honours ``%`` wildcards — passed through
            # verbatim. The escape clause here is purely a safety net so
            # that a literal backslash in the pattern doesn't cause an
            # invalid-pattern error.
            where.append("result LIKE :result_pattern ESCAPE '\\'")
        ...
        if q:
            ...
            if since is None:
                ...
                params["since"] = since
            from src.sql_safe import pg_like_escape
            where.append("CAST(params AS TEXT) LIKE :q ESCAPE '\\'")
