...
class SystemGroupProtected(Exception):
    """Raised when an attempted mutation would alter a system group.

    System groups (``Admin``, ``Everyone``) are seeded by the schema
    bootstrap with ``is_system=TRUE`` and must never be renamed,
    deleted, or have their ``is_system`` flag flipped.
    """
