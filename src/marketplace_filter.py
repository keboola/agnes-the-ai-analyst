...
the caller sees the distinct plugins granted to any of their groups. Admin
is just one of those groups here — there is no god-mode shortcut for the
marketplace feed, so admins curate their own view by granting plugins to
the Admin group. There is also no implicit Everyone — the caller's group
membership comes solely from real ``user_group_members`` rows.
...
def resolve_allowed_plugins(
    ...
    # Admin is treated as a regular group — admins get only the plugins
    # their groups have been granted.
...
def resolve_user_groups(
    ...
    Membership semantics mirror ``app.auth.access._user_group_ids``: only
    real ``user_group_members`` rows count, no implicit Everyone.
    """
    user_id = user.get("id")
    if not user_id:
        return []
