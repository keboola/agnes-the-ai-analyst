...
`app/api/query.py:_assert_select_only` rejects a string literal after
`FROM`/`JOIN` (`_FROM_STRING_LITERAL_RE`). Don't loosen it; don't add a new
query entrypoint that skips `_assert_select_only`.
