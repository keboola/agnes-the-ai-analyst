...
2. `<STATE_DIR>/system.duckdb.pre-migrate` is inspected read-only via
   ...
   running binary's `SCHEMA_VERSION` (currently `116`, in `src/db.py`).
...
# Expected: {"status": "ok", "db_schema": "ok", "current": 116, "expected": 116, ...}
