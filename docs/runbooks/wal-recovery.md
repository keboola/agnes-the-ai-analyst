...
2. `<STATE_DIR>/system.duckdb.pre-migrate` is inspected read-only via
   ...
   running binary's `SCHEMA_VERSION` (currently `86`, in `src/db.py`).
...
# Expected: {"status": "ok", "db_schema": "ok", "current": 86, "expected": 86, ...}
