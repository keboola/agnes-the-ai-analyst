...
| `cli/commands/analyst.py` | A | `da analyst setup` writes the hooks to `<workspace>/.claude/settings.json`. |
...
### Task A3: `da analyst setup` installs hooks into `<workspace>/.claude/settings.json`
...
```python
# Inside `_generate_claude_md` (or equivalent setup helper), after the
# workspace settings file is materialized:
...
**Note (design correction vs. earlier draft):** the hooks live at
**workspace level** (`<workspace>/.claude/settings.json`), not user-home.
User-home hooks would fire in *every* Claude Code session on the machine,
including unrelated workspaces where `da sync` would just produce stderr
noise. Workspace-level scoping keeps `da sync` running only when Claude
Code opens an Agnes-bootstrapped workspace.
...
Idempotently writes hooks into <workspace>/.claude/settings.json so each Claude
...
- `da analyst setup` now installs `SessionStart` (pull) and `SessionEnd` (upload) hooks into `<workspace>/.claude/settings.json`, idempotently, preserving any existing user-owned hooks.
...
`da analyst setup` installs Claude Code hooks that call `da sync --quiet` on every SessionStart and `da sync --upload-only --quiet` on SessionEnd, so analysts get fresh data without thinking about it. The hooks are in `<workspace>/.claude/settings.json` and you can edit them.
