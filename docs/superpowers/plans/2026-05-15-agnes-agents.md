...
Read the file. Verify section headings: `Required _meta table`, `Three connector shapes`, `_remote_attach table`, `Adding a new connector — checklist`, `Stable infrastructure`. Verify `connectors/jira/file_lock.py` and `connectors/jira/transform.py` exist (the "do not modify" list must reference real files).
