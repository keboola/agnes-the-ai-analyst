...
- **Flag-gated where it counts:** `access_policies.enabled` (config `access_policies.enabled` / env `AGNES_ACCESS_POLICIES_ENABLED`, `app/switches.py` flag `access_policies`) gates everything that ATTACHES or APPLIES a policy — `PUT /api/admin/registry/{id}` writing a non-null `access_policy_sql`, and the resolver substituting a policy on a read. Default OFF, so an instance that never turns it on never serves a policied row. The read-only **authoring helpers** are deliberately NOT flag-gated and match the posture `admin.py::preview_table_policy` already had: `GET /registry/{id}/policy/columns` and `POST /registry/{id}/policy/compile` are `Depends(require_admin)`, neither reads nor writes a stored policy (columns is a `DESCRIBE` + profile read; compile is a pure string generator), and the builder markup renders in the modal regardless — an admin can look at what a policy *would* say before enabling the feature, but cannot attach one. Keep the handler docstrings saying exactly this.
