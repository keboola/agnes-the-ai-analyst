...
| `keboola_token_header` | `auth.keboola.allow_token_header` | `AGNES_KEBOOLA_ALLOW_TOKEN_HEADER` | `false` | yes | Accept a Keboola Storage API token in the X-StorageApi-Token header as API authentication. Off by default: a plain Storage token carries no interactive factor, so enabling this bypasses any MFA/SSO the organization enforces on web logins. |
