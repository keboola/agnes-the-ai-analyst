...
| `ai_connector` | `ai_connector.enabled` | `AGNES_AI_CONNECTOR_ENABLED` | `true` | yes | "Connect your AI client" UI/instructions — `/me/ai-connector`, the `#connect` section on `/how-it-works` (and its two TOC rows), and `/mcp-connect`. On by default. Turn off on a VPN/intranet-only instance, where the generic OAuth MCP connector can never be reached by a cloud-based AI client (Claude.ai, ChatGPT) and the instructions would only mislead. Does not gate `/api/mcp/http` itself, or any nav link beyond those three pages — an in-network client can still connect. See `docs/DEPLOYMENT.md`'s tunnel section for the alternative that scopes external access to the agent-as-API surface instead. |
