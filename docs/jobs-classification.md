...
| `jira-org-refresh` | `POST /api/jobs` (`kind=jira-org-refresh`) | queued | One organization-API request per organization (minutes on a large site) — LIGHT lane, `daily 04:00`. No caller needs the result synchronously, so it never warranted a REST endpoint of its own. |
