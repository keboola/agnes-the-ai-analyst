...
that table, so a source alone would give it nothing. `mutating` comes from each
tool's `readOnlyHint`; a tool the upstream does not annotate is recorded as
...
A registration failure returns 502 and rolls back; a local config-write failure
propagates as a 500 rather than being dressed up as an upstream problem.
