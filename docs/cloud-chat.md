...
- **`ANTHROPIC_API_KEY` is gate-checked at startup.** Setting `chat.enabled: true` without `ANTHROPIC_API_KEY` will refuse to enable chat with a clear log line. Set the key in the Agnes server env before flipping the flag.
- **`sandbox_uid` and iptables OWNER rules are not cross-validated.** The operator must ensure the uid configured under `chat.sandbox_uid` matches the uid in iptables OWNER rules. Misconfiguration silently allows full egress from the sandbox. A future startup probe could compare configured `sandbox_uid` against `iptables -S OUTPUT | grep owner-uid`.
...
- **F.6 BigQuery budget assertion couples to `docker exec`.** The E2E test `tests/e2e/test_bq_budget.py::F.6` reads `app.api.query._per_session_bq_bytes` via in-container Python; if that dict is renamed the test breaks. Acceptable for an opt-in E2E suite gated on `AGNES_E2E_BQ=1`.
