...
| Experience preset — retired as a choice (Wave 0, 2026-08): `redesign` is the only valid value and the default; the old `classic` value (or any other unrecognised string) falls back to `redesign`. Historically flipped the DEFAULTS of `instance.theme` and `features.stack_auto_membership`; per-knob settings still win | `AGNES_INSTANCE_EXPERIENCE` | `instance.experience` | `redesign` | `get_experience()` |
