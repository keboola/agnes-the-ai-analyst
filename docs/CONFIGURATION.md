...
| Show the "connect your AI client" UI/instructions (`/me/ai-connector`, the `#connect` section on `/how-it-works`, `/mcp-connect`). `false` hides those three surfaces only — it does not gate `/api/mcp/http` itself or any other nav link. Meant for VPN/intranet-only instances where the generic OAuth MCP connector can never be reached by a cloud-based AI client, so the instructions would only mislead; see `docs/DEPLOYMENT.md`'s tunnel section for the alternative that scopes external access to the agent-as-API surface instead | `AGNES_AI_CONNECTOR_ENABLED` | `ai_connector.enabled` | `true` | `get_ai_connector_enabled()` |
