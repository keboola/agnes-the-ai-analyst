...
def agnes_data_app_preview(slug: str, url: str = "") -> dict:
    ...
    Returns ``{"render": "data_app_preview", "slug", "url", "preview_cookie"}``
    — ``url``/``preview_cookie`` are ``null`` on the placeholder call. Returns a
    ...
    try:
        grant = api_post_json(f"/api/data-apps/{slug}/preview-grant", {})
    except V2ClientError as exc:
