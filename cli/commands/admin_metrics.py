...
server-side metric registry (direct repo access, no API). Read paths
(list/show) live in `agnes catalog --metrics`.
"""
...
def import_metrics(
    ...
    """Import metric definitions from YAML into the metric registry."""
    import_path = Path(path)
    if not import_path.exists():
        typer.echo(f"Path not found: {path}", err=True)
        raise typer.Exit(1)

    count = metric_repo().import_from_yaml(import_path)
...
def export_metrics(
    ...
    """Export metric definitions to YAML files."""
    count = metric_repo().export_to_yaml(output_dir)
    typer.echo(f"Exported {count} metric(s) to {output_dir}")


@admin_metrics_app.command("validate")
def validate_metrics():
    """Check each metric's table reference against registered tables."""
