...
| Review a change before merge | `/agnes-review` | scope-gated review **team** (rules / adversarial always fire; architecture / rbac / parity fire only in-scope) + `agnes-review-consolidator` → one advisory report (`file:line` + severity, ≤15 findings). Read-only working tree; optional comment-only PR post. |
