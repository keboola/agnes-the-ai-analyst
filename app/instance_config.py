...
FEATURE_FLAGS: tuple[FeatureFlag, ...] = (
    ...
    FeatureFlag(
        ...
        default=False,
        description="Stack membership mode. Off (classic, the default): membership is the subscribe model — required plus subscribed grants — with the grant-downgrade subscription fan-out, exactly the pre-redesign behavior. On: auto-membership — every granted resource is in the stack immediately; subscribe/unsubscribe only control the local copy. The `instance.experience: redesign` preset defaults this to on.",
    ),
    FeatureFlag(
