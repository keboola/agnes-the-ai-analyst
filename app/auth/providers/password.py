...
def _set_login_cookie(response, user_id: str, email: str, role: str):
...
def _render_message(request: Request, title: str, message: str, status_code: int = 200):
    """Render a minimal themed message page via the main templates engine."""
    ...
    return templates.TemplateResponse(
        request, "_message.html", ctx, status_code=status_code,
    )
...
def _send_reset_email(request: Request, email: str, token: str) -> bool:
    ...
    try:
        ...
        return False
    reset_url = build_reset_url(request, email, token)
    return _send_mail(email, "Reset your password", f"Click to reset your password: {reset_url}")


def _send_setup_email(request: Request, email: str, token: str) -> bool:
    try:
        from app.auth.providers.email import is_available as email_available
        if not email_available():
            return False
        ...
        return False
    setup_url = build_setup_url(request, email, token)
    return _send_mail(email, "Set up your account", f"Click to set up your password: {setup_url}")
...
# ---- JSON programmatic setup (kept for backward compatibility) ----
...
async def reset_request(
    ...
    """Request a password-reset link. Always returns the same response (anti-enumeration)."""
...
async def setup_request(
    ...
    """Self-service 'Request Access' — emails a setup link if the user is pre-approved and unset."""
    email = (email or "").strip().lower()
    if email:
        repo = UserRepository(conn)
        user = repo.get_by_email(email)
        # Only issue setup token if user exists and has no password yet
        if user and not user.get("password_hash"):
            ...
            _send_setup_email(request, email, token)
    return _render_message(
        request,
        title="Check your email",
