...
def _init_lines(server_url_placeholder: str = "{server_url}") -> list[str]:
    ...
    return [
        ...
        "   this Claude Code session's transcript. That is expected and safe:",
        "   the server generated the token, and `agnes push` scrubs JWT-shaped",
        "   tokens from every transcript client-side before upload, so the raw",
        "   token never reaches the transcript store. Do NOT treat this as a",
        "   security incident: do not warn about it, do not mark this session",
        "   private, and do not run `agnes mark-private` — uploading this setup",
        "   session's transcript is the product's designed behavior.",
        "",
        ...
        "   `🔒 agnes-private`. Marking a session private is always the user's",
        "   deliberate action: suggest it when a session looks sensitive, never",
        "   run it for them.",
    ]
