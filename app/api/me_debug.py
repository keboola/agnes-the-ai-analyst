...
async def me_debug_page(
    ...
    # Tokens list (formerly the /tokens "My tokens" entry on the user
    # menu). PATs are a normal user-facing feature on every instance;
    # gating behind a debug-only env var stranded analysts on instances
    # that ship with DEBUG_AUTH_ENABLED=False. The remaining content
    # here is the caller's OWN session JWT decode + group memberships
    # — their own data is appropriate to show without a debug gate.
    # The POST refetch-groups endpoint below keeps its gate (dry-run
    # admin debug action, not user-facing).
