...
async def sync_manifest(
    ...
    homepage card.
    """
    # Skip the bookkeeping writes for co-session principals — they have no
    # persisted user row to stamp; the live session owns the pull. (From #528.)
    from app.auth.session_principal import SessionPrincipal
    if not isinstance(user, SessionPrincipal):
        ...
        # Goes through the factory (`usage_repo()`) so writes route to PG on
        # a PG-backed deploy — raw `UsageRepository(conn)` would lose this
        # signal on PG instances (#527 PG backend-split sweep).
        try:
