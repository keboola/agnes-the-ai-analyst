...
class CoworkZipError(Exception):
    """Raised when a plugin can't be packaged within Cowork's limits
    (file count / total size). The router maps this to HTTP 422."""
...
# Caps. Cowork rejects zips over 5000 files or 512 MB uncompressed total.
_MAX_FILES = 5000
_MAX_DEPTH = 10
...
# Built-zip cache (prefixed_name → (zip_bytes, etag)). Building a big plugin
# walks + concatenates thousands of files; without this every download (and
# every If-None-Match revalidation) repeats that work. Short TTL bounds drift
# vs on-disk content (marketplace sync is nightly). Mirrors packager's
# ETag-cache pattern.
...
def _emit_kv(key: str, value: Any) -> str:
    """Emit one ``key: value`` YAML line via the safe dumper.

    Using ``yaml.safe_dump`` (not f-string interpolation) means a value that
    looks like a non-string scalar (``true``, ``123``, ``null``), embeds a
    ``: ``/``#``, or opens with an indicator char (``[id]``, ``- x``) is
    quoted, so it round-trips back to the intended string. ``width`` is huge
    so descriptions stay on one line (matching the reference's plain scalars);
    ``allow_unicode`` keeps em-dashes raw rather than ``\\uXXXX`` escapes. The
    dumper uses single quotes when it must quote — never the double quotes
    Cowork's sanitizer rejects."""
...
def _emit_frontmatter(fields: List[Tuple[str, Any]], body: str) -> str:
    """Emit a clean frontmatter block + body. ``description`` is sanitized
    (``<``/``>`` removed, ``"``→``'``) before YAML-encoding."""
    lines = ["---"]
    for key, value in fields:
        if value is None:
            continue
        if key == "description":
            lines.append(_emit_kv("description", sanitize_description(value)))
        else:
...
def _transform_file(rel_parts: tuple, raw_bytes: bytes, plugin: dict) -> bytes:
    ...
    if rel_parts[0] == "skills" and rel_parts[-1].lower() == "skill.md":
        raw_folder = rel_parts[-2] if len(rel_parts) >= 2 else plugin["manifest_name"]
        # Name must equal the skill's zip directory, which is the SANITIZED
        # segment (e.g. `[id]` → `dyn-id`) — otherwise Cowork's folder==name
        # check fails.
        folder = sanitize_path_segment(raw_folder)
        try:
...
def collect_members(plugin: dict) -> List[Tuple[str, bytes]]:
    ...
    if plugin.get("bundle_dirs"):
        bases = [d.resolve() for d in plugin["bundle_dirs"] if d.is_dir()]
    ...
    if plugin_dir is not None and plugin_dir.is_dir():
        ...
        for f in sorted(p for p in plugin_dir.rglob("*") if p.is_file()):
            ...
            if _is_stripped(rel_parts):
                continue
            # Symlink guard (#1) — a hostile plugin could symlink to host files
            # (rglob may also descend symlinked dirs); never read a path that
...
def build_cowork_zip(plugin: dict) -> Tuple[bytes, str]:
    ...
    Raises ``CoworkZipError`` if the plugin can't fit Cowork's caps."""
...
def invalidate_cache() -> None:
    """Drop all cached zips — call after a marketplace sync changes on-disk
    plugin content so the next download re-builds against the new files."""
