...
def make_git_wsgi_app() -> Callable:
    ...
    def app(environ: dict, start_response: Callable) -> Iterable[bytes]:
        ...
        # resolve_token_to_user / ensure_repo_for_user route every state
        # read through the src.repositories factory — `conn` is retained
        # only for signature stability (see the matching notes on each),
        # so no DuckDB connection needs to be opened here. Opening one
        # unconditionally force-held the process-singleton system.duckdb
        # connection on every git-fetch request, even on Postgres-backed
        # instances.
        conn = None
        ...
        try:
            ...
            logger.exception("Failed to open repo for user %r", user.get("email") or user.get("id"))
