...
- ~~**Per-table outbound tools** not implemented~~ Done — REST surface: `POST /api/mcp/query-table/{table_id}` with body `{filter: {col: val, …}, limit: int}` returns a parameterized `SELECT * FROM "{view}" WHERE col = ? … LIMIT N` against the orchestrator's view in `analytics.duckdb`. Filter columns are validated against the actual view schema (read via `DESCRIBE`); unknown keys 400 with the allow-list so AI clients self-correct. Limit caps at 1000 with a `truncated` flag. RBAC piggybacks on `ResourceType.TABLE` so the existing table grants are reused — admin short-circuits. A follow-up commit can register one FastMCP tool per `table_registry` row on both the HTTP/SSE endpoint and the stdio server so the REST shape surfaces as `query_<table_id>(filter=…)` natively.
- **Vault — both scopes done**: `app/secrets_vault.py` covers two tables. (a) `mcp_secrets` (v62) — server-wide secret, one row per source, managed by admin (`PUT /api/admin/mcp-sources/{id}/secret`, `agnes admin mcp source set-secret`). (b) `mcp_user_secrets` (v63) — per-(source, user) secret, managed by the analyst (`PUT /api/mcp/sources/{id}/my-secret`, `agnes mcp my-secret set`). The new `mcp_sources.scope` column ('shared' | 'per_user') selects which path `_lookup_secret_for_source(source, caller_user_id=...)` follows: per_user prefers the caller's row and falls back to shared when absent. `call_tool_async` threads the caller's user_id; scheduled materialize jobs leave it `None` and always use shared (they have no calling user). Production deployments set `AGNES_VAULT_KEY` (URL-safe-base64 32-byte Fernet key); without it the vault uses an ephemeral key + logs a warning. Cleartext is never returned by either GET surface — rotation is write-only.
- ~~**No Policy Engine** — read-only is implicit.~~ Done: `app/api/mcp_policy.py` carries three gates wired into `/api/mcp/passthrough/tools/{id}/call`. (1) `tool_registry.mutating=true` is admin-only (analysts get 403). (2) `tool_registry.pii_fields` (list[str]) drives recursive redaction of upstream JSON output before it leaves the server — `text` and `data` stay in sync. (3) `tool_registry.rate_limit_pm` (int) caps calls per-minute per-(tool, user) via an in-memory token bucket; over-quota returns 429 with a Retry-After header. Bucket state is per-process and clears on restart (POC choice — RFC §3 may move it to Redis once we run multi-replica). Admin REST + UI to set these columns ship in the next iteration; for now use `agnes admin mcp tool register --mutating/--pii-fields/--rate-limit-pm` on the CLI.
...
- ~~**data_packages `related_tools` junction**~~ Done: `data_package_tools(package_id, tool_id, added_at)` (v64). Admin REST: `POST /api/admin/data-packages/{id}/tools` + `DELETE /api/admin/data-packages/{id}/tools/{tool_id}` (mirroring the `/tables` shape). Detail endpoints — `GET /api/admin/data-packages/{id}` (admin) and `GET /api/data-packages/{slug}` (analyst) — now carry a `related_tools` array enriched with `source_name` + `mode` so UIs can render the tool chip alongside the table chip.
