...
- **Vault — shared scope done**: `app/secrets_vault.py` + `mcp_secrets` table (v62) store Fernet-encrypted server-wide tokens. `connectors/mcp/client._lookup_secret_for_source` consults the vault first, then falls back to `auth_secret_env` — flipping a source from env-var to vault is `agnes admin mcp source set-secret <name>`. Production deployments set `AGNES_VAULT_KEY` (URL-safe-base64 32-byte Fernet key); without it the vault uses an ephemeral key + logs a warning. **Per-user credential passthrough still TODO** — each analyst storing their own Notion/Slack/Linear OAuth token so calls forward under their identity needs a `mcp_user_secrets` table (v63) + the user_id threaded through `call_tool_async`.
