...
### Added
- MCP source secrets are now fully manageable from the admin UI: set/rotate/clear a vault-stored secret (encrypted at rest), an `env` (`KEY=VALUE`) field and a `scope` selector on the source form, and a secret-status indicator — no host environment variable required. The legacy `auth_secret_env` (host-env) path is relabelled "Advanced (legacy)" and still works. Storing a secret with no `AGNES_VAULT_KEY` set now returns `409` (outside `LOCAL_DEV_MODE`) instead of silently using an ephemeral key that loses the value on restart; a set-but-invalid key still raises a clear config error. `GET /api/health` now reports `vault_key_configured`, source serialization includes a `has_vault_secret` boolean, and `AGNES_VAULT_KEY` is documented in `config/.env.template`. Deleting a source now also removes its vault secrets (shared + per-user) so no orphaned encrypted rows are left behind. (#530)

### Known limitations
- The vault repositories (`SharedSecretsRepository`, `PerUserSecretsRepository` in `app/secrets_vault.py`) still operate on a raw DuckDB connection only. On a Postgres state backend (CLOUD / SIDE_CAR), the Alembic `0014` `mcp_secrets` / `mcp_user_secrets` tables exist but no code writes to them — secrets land in the DuckDB file no reader sees. Same dual-backend drift class as #517 / #522 / #520's Devin finding; pre-existing (the vault landed in #474 before the backend-aware factory was the standard) and called out by Devin Review on this PR. Tracked as a follow-up; Postgres-backed operators should keep using the legacy `auth_secret_env` host-env path until a vault PG sibling lands.
