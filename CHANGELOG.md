...
- Postgres-backed instances: the admin telemetry export (`GET /api/admin/telemetry/export`) and telemetry Ask (`POST /api/admin/telemetry/ask`) executed their SQL on the always-DuckDB request connection, silently exporting/answering from the orphaned DuckDB file (zero or frozen pre-cutover rows). Both now read through the backend-aware repository factory; Ask additionally prompts the LLM for the active backend's SQL dialect and its function denylist covers PostgreSQL escape hatches (`pg_read_file`, `dblink`, …). Parquet export is built in memory with pyarrow (engine-agnostic) instead of DuckDB `COPY`.
