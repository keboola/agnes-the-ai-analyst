...
- **`agnes-state-applier.service` self-bootstraps its non-root user via a paired bootstrap unit.** A dedicated `agnes-state-applier-bootstrap.service` runs as root (no `User=` directive) before the main unit, creating the `agnes-applier` user, adding it to the `docker` group, and chowning `/data/state`. The main unit `Requires=` it, so by the time systemd loads `User=agnes-applier` on the main unit, the user exists. An earlier attempt put the bootstrap in `ExecStartPre=+` of the main unit; verified live on `foundryai-dev-zsrotyr` that systemd validates `User=` at unit LOAD time, refusing to start with "Failed to determine user credentials" before any `ExecStartPre` ran. Customer infras that maintain their own provisioning scripts get the Phase 8.1 non-root posture without shipping matching user-creation logic.
