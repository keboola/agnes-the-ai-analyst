...
- **`agnes-state-applier.service` self-bootstraps its non-root user.** The unit's `ExecStartPre=+` directives (`+` runs as root) create `agnes-applier`, add it to the `docker` group, and chown `/data/state` on first run. Customer infras that maintain their own provisioning scripts (e.g. forks that don't consume the OSS `customer-instance` module) get the Phase 8.1 non-root posture without shipping matching user-creation logic; the OSS startup script still carries the eager block as belt-and-braces.
