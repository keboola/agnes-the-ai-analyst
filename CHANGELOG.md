...
### Fixed
- **Admin-installed curated plugins were invisible in the served marketplace clone AND in My-Stack views.** `POST /api/marketplace/curated/<m>/<p>/install`'s `require_resource_access` gate calls `app.auth.access.can_access`, which short-circuits to True for any user in the Admin group regardless of explicit `resource_grants` — admin god-mode lets an admin subscribe to ANY `marketplace_plugins` row. The aggregate `resolve_user_marketplace` AND every UI surface listing the user's stack (`/api/marketplace?tab=my`, `/api/my-stack`, category counts, analyst CLAUDE.md render) then composed `(rbac ∩ subscriptions)` strictly through `resolve_allowed_plugins`, which had no admin bypass. Net result: admin clicks "Add to stack" → `agnes marketplace search` reports `installed: true` → `~/.agnes/marketplace/.claude-plugin/marketplace.json` from `/marketplace.git/` has only plugins admin's group explicitly granted, AND the web /marketplace?tab=my view shows the same gap, AND `agnes refresh-marketplace` never sees the new subscription. Fix: `resolve_allowed_plugins(conn, user, *, admin_bypass=True)` — defaults to `True`, matches the install endpoint's `can_access` semantic, returns ALL `marketplace_plugins` rows for admin callers. Short-circuits the strict SQL join entirely on the hot path (eliminates the discard-and-re-scan pattern flagged by Codex H-3). All existing callers (My-Stack views, category counts, served set composer, claude_md renderer) pick up the new default automatically. Strict-mode semantic preserved as `admin_bypass=False` for "explicit grant graph" introspection. Tests: `test_admin_strict_mode_filters_through_grants` (pins the strict path), `test_admin_default_bypass_returns_all_plugins` (pins the new default), `test_admin_subscriptions_surface_without_explicit_grants` (regression for the original symptom), `test_non_admin_still_needs_explicit_grant` (confirms non-admins are unaffected).
