...
### Added
- Authoring agents — non-admin suggestion queue (`authoring_suggestions`, DuckDB v77 + Alembic, dual-backend): `POST /api/studio/suggestions` lets a non-admin submit a proposed create payload per studio domain; admins review via a moderation queue at `/admin/studio/suggestions` + `GET/POST /api/admin/authoring-suggestions[/{id}/approve|reject]`. Approving a `data-package` or `corporate-memory` suggestion auto-creates the real resource; `mcp`/`marketplace` are excluded from auto-replay (their `command`/`url` payloads are a confused-deputy vector) and stay manual-create.
