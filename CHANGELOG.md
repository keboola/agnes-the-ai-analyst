...
- `AGNES_GOOGLE_GROUP_PREFIX` env var filters which Google Workspace groups Agnes mirrors into `user_groups` and gates Google logins. When set (e.g. `grp_foundryai_`), only Workspace groups whose email starts with that prefix become Agnes groups, and Google logins by users with no matching membership are rejected with `?error=not_in_foundryai_group`. Default `""` preserves legacy behavior. See `docs/auth-groups.md` § "Group prefix filter and external linking".
- `user_groups.external_id` column links an Agnes group to a Google Workspace group email. Set automatically by sync; immutable via UI once written. The admin UI shows it as a copyable green chip on `/admin/groups` and surfaces a read-only banner on `/admin/groups/{id}` for bound groups.
- `app/auth/group_sync.fetch_user_groups` now distinguishes soft-fail (`None`) from a successful zero-group response (`[]`) so the OAuth callback can deny first-time logins when group membership is unverifiable while still passing-through returning users with cached membership during a transient Google API outage.

### Changed

- **BREAKING** The `Everyone` system group is no longer auto-assigned to every user. Membership now comes from being a member of `<AGNES_GOOGLE_GROUP_PREFIX>everyone@<domain>` (Google logins) or from explicit admin assignment (email logins). The v13 `system_seed` Everyone rows from the v13 backfill are deleted by the v15 migration; affected Google users re-acquire Everyone on their next sign-in if the prefix-everyone group is configured. Email-only users keep zero implicit group membership and need explicit admin assignment.
- **BREAKING** Admin UI and API can no longer add or remove members on Google-bound groups (those with `external_id` set). Membership for those groups is sourced exclusively from Google Workspace. The membership endpoints under `/api/admin/groups/{id}/members` and `/api/admin/users/{id}/memberships` return `409 Conflict { "code": "external_group_readonly" }` for blocked admin-source mutations. The `replace_google_sync_groups` (Google-sync) and `source='system_seed'` (SEED_ADMIN_EMAIL bootstrap) writers bypass the guard intentionally. Admins can still rename the display `name` of non-system bound groups; the `external_id` link itself is immutable.
- Google login error reporting renders an inline banner on `/login` mapping query-string codes (`not_in_foundryai_group`, `group_check_unavailable`, `domain_not_allowed`, `deactivated`, `oauth_failed`, `google_not_configured`, `no_email`) to human text, instead of silently dropping users back to the login form with no explanation.

### Internal

- Schema v15 — adds `user_groups.external_id` (`VARCHAR UNIQUE NULL`). Migration removes orphan email-named google-synced groups (rows with `created_by='system:google-sync' AND external_id IS NULL AND name LIKE '%@%' AND id NOT IN (SELECT group_id FROM resource_grants)`) and stale `system_seed` Everyone rows tagged `system:v13-backfill`. Cleanup is wrapped in `BEGIN/COMMIT` and protected against deleting any group still referenced from `resource_grants`.
- New helpers: `UserGroupsRepository.get_by_external_id`, `attach_external_id`, `resolve_or_create_for_external` (lookup-by-external_id → lookup-by-derived-name promote → create); new exceptions `ExternalIdConflict` and `ExternalIdImmutable`. New helper `UserGroupMembersRepository.has_any_google_sync_membership` and a `_guard_external` hook on `add_member`/`remove_member`; new exception `ExternalGroupReadOnly`.
- New test module `tests/test_user_groups_external.py` covers schema v15, derivation, promote, conflict, and the membership guard. `tests/test_group_sync.py` updated for the `Optional[list]` return type.
