...
- **`/admin/audit` page** (admin-only) + **`GET /api/audit`** + **`GET /api/audit/actions`** endpoints: browser surface for the `audit_log` table. Filter by category (script.* / metric.* / sync.* / upload.* / registry.* / instance.* / permission.* / access_request.* / metadata.* / catalog.* / km_*), specific action, user_id, or resource. Limit clamped 1–500. Replaces the previously-removed `/activity-center` route — instead of rendering against undefined stub fields (the old template referenced `executive_summary`, `maturity_roadmap`, `business_processes` which no service produces), this is the operator-facing window into the real audit trail produced by the per-module `_audit()` instrumentation. `AuditRepository.query()` extended with `action_prefix` (LIKE) and `resource` filters to back the page.
...
- `tests/test_admin_audit.py` — 11 tests pinning the `/admin/audit` page render + the `GET /api/audit` and `GET /api/audit/actions` endpoint contracts (filters, limit clamp, RBAC, distinct-actions list).
...
- `/activity-center` page, its dashboard widget, and the `test_activity_center` smoke test. The route was rendering against undefined stub data passed by `app/web/router.py` (`_SilentUndefined` masked the breakage), and no service produces the `executive_summary` / `maturity_roadmap` / `business_processes` fields the template referenced. **Replaced by `/admin/audit`** which renders the real `audit_log` table with category/action/user/resource filters.
