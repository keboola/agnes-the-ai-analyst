...
- **`setup.py` updates `.claude/settings.json` with absolute paths** — replaces the relative `python3 mcp_server.py` with `sys.executable` + absolute launcher path so MCP starts correctly regardless of working directory.
