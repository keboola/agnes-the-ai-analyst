...
- **BREAKING (API response):** `_build_telemetry` returns `None` (not
  a zero-shape dict) when `invocations_30d == 0`, so the `telemetry`
  field on `/api/marketplace/curated/{mp}/{plugin}`,
  `/api/marketplace/flea/{id}`, and the inner skill/agent detail
  endpoints may now be `null` where the previous shape was always a
  dict. The frontend hero / sidebar are already None-safe
  (`d.telemetry || {}` guard, `if (!d.telemetry || …)` on
  daily_series). External consumers reading these endpoints must
...
- `src/usage_ask.py` — `SCHEMA_DIGEST` + `SYSTEM_PROMPT` updated to
  describe the new v48 rollup tables (`usage_marketplace_item_daily`,
  `usage_marketplace_item_window`) instead of the dropped
  `usage_plugin_daily`. Admin "ask telemetry" feature emits SQL
  against the live schema again.
...
- `tests/conftest.py` — dropped the unused
  ...
  the now-removed `usage_attribution_*` tables. It had zero callers
  but was a future tripwire (next consumer would have hit a binder
  error).
